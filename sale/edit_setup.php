...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  try {
    ...
    redirect_to(
      app_system_url(
        'sale/setups.php?status=updated&id=' . urlencode($setupId)
      )
    );
  } catch (Throwable $e) {
    try {
...
<div class="sales-dashboard-page">
  <div class="setup-page setup-work-page setup-create-page setup-edit-page">
        ...
        <section class="setup-main-panel">
          ...
          <div class="setup-tab-panel active" id="customerPanel">
            ...
            <div class="setup-search-wrap">
              ...
              <input type="search" id="customerSearch" placeholder="พิมพ์ชื่อ เบอร์โทร หรือรหัสลูกค้า...">
            </div>
          ...
          <div class="setup-tab-panel" id="productsPanel">
            <div class="setup-section-heading product-heading-row">
              ...
              <div class="setup-search-wrap product-search">
                <?= edit_setup_icon('search') ?>
                <input type="search" id="productSearch" placeholder="ค้นหาสินค้า หรือประเภทสินค้า...">
              </div>
            </div>
        ...
        <aside class="setup-summary-sidebar">
          <div class="summary-card sticky-summary">
            <div class="summary-title-row">
              <div>
                <h2>สรุปใบงาน</h2>
                <p>ตรวจสอบข้อมูลก่อนบันทึกการแก้ไข</p>
              </div>
            ...
            <div class="summary-customer-card" id="summaryCustomer">
              ...
              <div class="summary-customer-details" id="summaryCustomerDetails" hidden>
            ...
            <div class="summary-form-section">
              ...
              <textarea id="setupAddress" name="setup_address" rows="4"
                placeholder="กรอกบ้านเลขที่ หมู่ ถนน ตำบล อำเภอ จังหวัด และรหัสไปรษณีย์" required></textarea>
              ...
              <textarea id="setupNote" name="setup_note" rows="3"
                placeholder="รายละเอียดเพิ่มเติมสำหรับงานติดตั้ง (ถ้ามี)"></textarea>
            </div>

            <button type="submit" class="setup-submit-btn" id="submitSetupBtn" disabled>
              <?= edit_setup_icon('save') ?>
              บันทึกการแก้ไข
            </button>

            <p class="summary-submit-note" id="summarySubmitNote">
              ระบบจะคงข้อมูลเดิมไว้จนกว่าจะมีการแก้ไข
            </p>
          </div>
        </aside>
      </div>
    ...
    <div class="product-detail-modal" id="productDetailModal" hidden>
      ...
      <div class="product-detail-dialog" role="dialog" aria-modal="true" aria-labelledby="modalProductName">
        <button type="button" class="product-modal-close" data-close-modal aria-label="ปิดรายละเอียดสินค้า">
          ×
        </button>
...
<script
  src="<?= h(app_asset_url('sale/assets/js/create_setup.js')) ?>?v=<?= h(asset_version('sale/assets/js/create_setup.js')) ?>"></script>
