...
function get_customer_by_id(mysqli $conn, string $user_id): ?array
{
    $stmt = $conn->prepare("\n        SELECT *\n        FROM `user`\n        WHERE user_id = ?\n          AND user_role = 0\n        LIMIT 1\n    ");
...
$customer = get_customer_by_id($conn, $user_id);
    ...
    try {
        ...
        if ($check_result->num_rows > 0) {
            ...
            if ($duplicate['user_name'] === $user_name) {
                redirect_to(app_system_url('admin/customer_edit.php?id=' . urlencode($user_id) . '&status=duplicate_name'));
            }

            if ($duplicate['user_phone'] === $user_phone) {
                redirect_to(app_system_url('admin/customer_edit.php?id=' . urlencode($user_id) . '&status=duplicate_phone'));
            }

            if ($duplicate['user_email'] === $user_email) {
                redirect_to(app_system_url('admin/customer_edit.php?id=' . urlencode($user_id) . '&status=duplicate_email'));
            }

            redirect_to(app_system_url('admin/customer_edit.php?id=' . urlencode($user_id) . '&status=duplicate'));
        }

        if ($user_password !== '') {
            $stmt = $conn->prepare("\n                UPDATE `user`\n                SET user_name = ?,\n                    user_fullname = ?,\n                    user_password = ?,\n                    user_phone = ?,\n                    user_email = ?,\n                    user_address = ?\n                WHERE user_id = ?\n                  AND user_role = 0\n            ");

            $stmt->bind_param('sssssss', $user_name, $user_fullname, $user_password, $user_phone, $user_email, $user_address, $user_id);
        } else {
            $stmt = $conn->prepare("\n                UPDATE `user`\n                SET user_name = ?,\n                    user_fullname = ?,\n                    user_phone = ?,\n                    user_email = ?,\n                    user_address = ?\n                WHERE user_id = ?\n                  AND user_role = 0\n            ");

            $stmt->bind_param('ssssss', $user_name, $user_fullname, $user_phone, $user_email, $user_address, $user_id);
        }
...
layout_header('แก้ไขข้อมูลลูกค้า', 'customers');
?>
<?= flash_message() ?>
<div class="staff-form-page">
  <div class="staff-page-back-row"><a class="staff-back-link" href="<?= h(app_system_url('admin/customers.php')) ?>"><?= admin_form_icon_svg('back') ?> กลับรายการลูกค้า</a></div>
  <form class="staff-create-card" method="POST" action="<?= h(app_system_url('admin/customer_edit.php')) ?>" autocomplete="off">
    <input type="hidden" name="user_id" value="<?= h($customer['user_id']) ?>">
    <div class="staff-create-head staff-create-head-clean"><div><h2>แก้ไขข้อมูลลูกค้า</h2></div><div class="staff-password-pill"><?= admin_form_icon_svg('lock') ?> เว้นรหัสผ่านว่างไว้ หากไม่ต้องการเปลี่ยน</div></div>
    <div class="staff-form-grid">
      <div class="staff-field readonly-field"><label for="user_id_show">รหัสลูกค้า *</label><div class="staff-input-wrap"><?= admin_form_icon_svg('id') ?><input type="text" id="user_id_show" value="<?= h($customer['user_id']) ?>" readonly></div></div>
      <div class="staff-field readonly-field"><label>สิทธิ์การใช้งาน</label><div class="staff-input-wrap"><?= admin_form_icon_svg('role') ?><input type="text" value="ลูกค้า" readonly></div></div>
      <div class="staff-field"><label for="user_name">ชื่อผู้ใช้ *</label><div class="staff-input-wrap"><?= admin_form_icon_svg('user') ?><input type="text" id="user_name" name="user_name" maxlength="50" value="<?= h($customer['user_name']) ?>" required></div></div>
      <div class="staff-field"><label for="user_fullname">ชื่อ-นามสกุลจริง *</label><div class="staff-input-wrap"><?= admin_form_icon_svg('user') ?><input type="text" id="user_fullname" name="user_fullname" maxlength="100" value="<?= h($customer['user_fullname'] ?? '') ?>" required></div></div>
      <div class="staff-field"><label for="user_phone">เบอร์โทรศัพท์ *</label><div class="staff-input-wrap"><?= admin_form_icon_svg('phone') ?><input type="text" id="user_phone" name="user_phone" maxlength="10" inputmode="numeric" value="<?= h($customer['user_phone']) ?>" required></div></div>
      <div class="staff-field"><label for="user_email">อีเมล Gmail *</label><div class="staff-input-wrap"><?= admin_form_icon_svg('mail') ?><input type="email" id="user_email" name="user_email" maxlength="50" value="<?= h($customer['user_email']) ?>" required></div></div>
      <div class="staff-field staff-field-full"><label for="user_password">รหัสผ่าน</label><div class="staff-input-wrap"><?= admin_form_icon_svg('lock') ?><input type="password" id="user_password" name="user_password" maxlength="50" placeholder="กรอกรหัสผ่านใหม่เมื่อต้องการเปลี่ยน"></div></div>
      <input type="hidden" id="user_address" name="user_address" value="<?= h($customer['user_address']) ?>">
      <div class="staff-field staff-field-full staff-address-section"><div class="staff-address-title"><?= admin_form_icon_svg('map') ?><label>ข้อมูลที่อยู่ *</label></div>
        <div class="staff-address-grid">
          <div class="staff-field staff-address-detail"><label for="address_detail">บ้านเลขที่ / หมู่ / ถนน *</label><div class="staff-input-wrap"><?= admin_form_icon_svg('map') ?><input type="text" id="address_detail" name="address_detail" maxlength="255" placeholder="เช่น 150 หมู่ 1 ถนนศรีจันทร์" value="<?= h($customer['user_address']) ?>" required></div></div>
          <div class="staff-field"><label for="province_id">จังหวัด *</label><div class="staff-input-wrap address-select-wrap"><?= admin_form_icon_svg('map') ?><select id="province_id" name="province_id" class="js-address-select"><option value="" selected disabled>เลือกจังหวัด</option><?php foreach ($provinces as $province): ?><option value="<?= h($province['id']) ?>"><?= h($province['name_th']) ?></option><?php endforeach; ?></select></div></div>
          <div class="staff-field"><label for="district_id">อำเภอ *</label><div class="staff-input-wrap address-select-wrap"><?= admin_form_icon_svg('map') ?><select id="district_id" name="district_id" class="js-address-select" disabled><option value="" selected>เลือกจังหวัดก่อน</option></select></div></div>
          <div class="staff-field"><label for="sub_district_id">ตำบล *</label><div class="staff-input-wrap address-select-wrap"><?= admin_form_icon_svg('map') ?><select id="sub_district_id" name="sub_district_id" class="js-address-select" disabled><option value="" selected>เลือกอำเภอก่อน</option></select></div></div>
          <div class="staff-field"><label for="zip_code">รหัสไปรษณีย์ *</label><div class="staff-input-wrap readonly-field"><?= admin_form_icon_svg('id') ?><input type="text" id="zip_code" name="zip_code" maxlength="10" placeholder="ระบบเติมให้อัตโนมัติ" readonly></div></div>
        </div>
      </div>
    </div>
    <div class="staff-form-actions"><a class="staff-cancel-btn" href="<?= h(app_system_url('admin/customers.php')) ?>">ยกเลิก</a><button class="staff-save-btn" type="submit"><?= admin_form_icon_svg('save') ?> บันทึกข้อมูล</button></div>
  </form>
</div>
