...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($user_id === '' || $user_name === '' || $user_fullname === '' || $user_phone === '' || $user_email === '' || $address_detail === '' || $province_id <= 0 || $district_id <= 0 || $sub_district_id <= 0 || $zip_code === '' || $user_role === '') {
  ...
  try {
    ...
    $user_address = $address_detail . ' ต.' . $addr['sub_district_name'] . ' อ.' . $addr['district_name'] . ' จ.' . $addr['province_name'] . ' ' . $zip_code;

    $check = $conn->prepare("SELECT user_id, user_name, user_phone, user_email FROM `user` WHERE user_id = ? OR user_name = ? OR user_phone = ? OR user_email = ? LIMIT 1");
    ...
    if ($check_result->num_rows > 0) {
      ...
      if ($duplicate['user_id'] === $user_id) redirect_to(app_system_url('admin/user_add.php?status=duplicate_id'));
      if ($duplicate['user_name'] === $user_name) redirect_to(app_system_url('admin/user_add.php?status=duplicate_name'));
      if ($duplicate['user_phone'] === $user_phone) redirect_to(app_system_url('admin/user_add.php?status=duplicate_phone'));
      if ($duplicate['user_email'] === $user_email) redirect_to(app_system_url('admin/user_add.php?status=duplicate_email'));
      redirect_to(app_system_url('admin/user_add.php?status=duplicate'));
    }

    $stmt = $conn->prepare("INSERT INTO `user` (user_id, user_name, user_fullname, user_password, user_phone, user_email, user_address, user_role) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param('sssssssi', $user_id, $user_name, $user_fullname, $user_password, $user_phone, $user_email, $user_address, $user_role);
    $stmt->execute();
...
<div class="staff-form-page">
  <div class="staff-page-back-row">
    <a class="staff-back-link" href="<?= h(app_system_url('admin/users.php')) ?>"><?= icon_svg('back') ?> กลับรายการพนักงาน</a>
  </div>

  <form class="staff-create-card" method="POST" action="<?= h(app_system_url('admin/user_add.php')) ?>" autocomplete="off">
    <div class="staff-create-head staff-create-head-clean">
      <div><h2>ข้อมูลพนักงานใหม่</h2></div>
      <div class="staff-password-pill"><?= icon_svg('lock') ?> รหัสผ่านเริ่มต้น: <strong>1234</strong></div>
    </div>

    <div class="staff-form-grid">
      <div class="staff-field readonly-field"><label for="user_id">รหัสผู้ใช้ *</label><div class="staff-input-wrap"><?= icon_svg('id') ?><input type="text" id="user_id" name="user_id" maxlength="13" value="<?= h($default_user_id) ?>" readonly required></div><small>ระบบสร้างรหัสให้อัตโนมัติ</small></div>

      <div class="staff-field"><label for="user_role">สิทธิ์การใช้งาน *</label><div class="staff-input-wrap select-wrap"><?= icon_svg('role') ?><select id="user_role" name="user_role" required><option value="" selected disabled>เลือกสิทธิ์การใช้งาน</option><option value="1">หัวหน้าช่าง</option><option value="2">พนักงานขาย</option><option value="3">ผู้ดูแลระบบ</option></select></div><small>เลือกหน้าที่ของพนักงานในระบบ</small></div>

      <div class="staff-field"><label for="user_name">ชื่อผู้ใช้ *</label><div class="staff-input-wrap"><?= icon_svg('user') ?><input type="text" id="user_name" name="user_name" maxlength="50" placeholder="เช่น somchai หรือ admin2" required></div><small>ใช้สำหรับแสดงชื่อในระบบ และต้องไม่ซ้ำ</small></div>

      <div class="staff-field"><label for="user_fullname">ชื่อ-นามสกุลจริง *</label><div class="staff-input-wrap"><?= icon_svg('user') ?><input type="text" id="user_fullname" name="user_fullname" maxlength="100" placeholder="เช่น สมชาย ใจดี" required></div><small>ใช้สำหรับตรวจสอบตัวตนของพนักงาน</small></div>

      <div class="staff-field"><label for="user_phone">เบอร์โทรศัพท์ *</label><div class="staff-input-wrap"><?= icon_svg('phone') ?><input type="text" id="user_phone" name="user_phone" maxlength="10" inputmode="numeric" placeholder="0812345678" required></div><small>กรอกตัวเลข 10 หลักเท่านั้น</small></div>

      <div class="staff-field"><label for="user_email">อีเมล Gmail *</label><div class="staff-input-wrap"><?= icon_svg('mail') ?><input type="email" id="user_email" name="user_email" maxlength="50" placeholder="example@gmail.com" required></div><small>รองรับเฉพาะอีเมล @gmail.com</small></div>

      <div class="staff-field staff-field-full staff-address-section"><div class="staff-address-title"><?= icon_svg('map') ?><label>ข้อมูลที่อยู่ *</label></div>
        <div class="staff-address-grid">
          <div class="staff-field staff-address-detail"><label for="address_detail">บ้านเลขที่ / หมู่ / ถนน *</label><div class="staff-input-wrap"><?= icon_svg('map') ?><input type="text" id="address_detail" name="address_detail" maxlength="255" placeholder="เช่น 150 หมู่ 1 ถนนศรีจันทร์" required></div></div>

          <div class="staff-field"><label for="province_id">จังหวัด *</label><div class="staff-input-wrap address-select-wrap"><?= icon_svg('map') ?><select id="province_id" name="province_id" class="js-address-select" required><option value="" selected disabled>เลือกจังหวัด</option><?php foreach ($provinces as $province): ?><option value="<?= h($province['id']) ?>"><?= h($province['name_th']) ?></option><?php endforeach; ?></select></div></div>

          <div class="staff-field"><label for="district_id">อำเภอ *</label><div class="staff-input-wrap address-select-wrap"><?= icon_svg('map') ?><select id="district_id" name="district_id" class="js-address-select" required disabled><option value="" selected>เลือกจังหวัดก่อน</option></select></div></div>

          <div class="staff-field"><label for="sub_district_id">ตำบล *</label><div class="staff-input-wrap address-select-wrap"><?= icon_svg('map') ?><select id="sub_district_id" name="sub_district_id" class="js-address-select" required disabled><option value="" selected>เลือกอำเภอก่อน</option></select></div></div>

          <div class="staff-field"><label for="zip_code">รหัสไปรษณีย์ *</label><div class="staff-input-wrap readonly-field"><?= icon_svg('id') ?><input type="text" id="zip_code" name="zip_code" maxlength="10" placeholder="ระบบเติมให้อัตโนมัติ" readonly required></div></div>
        </div>
      </div>
    </div>

    <div class="staff-form-actions"><a class="staff-cancel-btn" href="<?= h(app_system_url('admin/users.php')) ?>">ยกเลิก</a><button class="staff-save-btn" type="submit"><?= icon_svg('save') ?> เพิ่มข้อมูลพนักงาน</button></div>
  </form>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function closeAddressDropdowns(except) { document.querySelectorAll('.address-dropdown.is-open').forEach(function (dropdown) { if (dropdown !== except) dropdown.classList.remove('is-open'); }); }
  function refreshAddressDropdown(select) { if (select && typeof select._refreshAddressDropdown === 'function') select._refreshAddressDropdown(); }
  ...
  function initAddressDropdown(select) {
    ...
    function render() {
      ...
      Array.from(select.options).forEach(function (option) {
        ...
        item.addEventListener('click', function () { if (item.disabled) return; select.value = option.value; closeAddressDropdowns(); dropdown.classList.remove('is-open'); select.dispatchEvent(new Event('change', { bubbles: true })); render(); });
        menu.appendChild(item);
      });
    }

    button.addEventListener('click', function () { if (select.disabled) return; const willOpen = !dropdown.classList.contains('is-open'); closeAddressDropdowns(dropdown); dropdown.classList.toggle('is-open', willOpen); });
  ...
  function loadDistricts(provinceId) {
    ...
    fetch(`${districtUrl}?province_id=${encodeURIComponent(provinceId)}`)
      ...
      .then(data => { resetSelect(districtSelect, 'เลือกอำเภอ', false); data.forEach(item => { const option = document.createElement('option'); option.value = item.id; option.textContent = item.name_th; districtSelect.appendChild(option); }); refreshAddressDropdown(districtSelect); })
  ...
  function loadSubDistricts(districtId) {
    ...
    fetch(`${subDistrictUrl}?district_id=${encodeURIComponent(districtId)}`)
      .then(response => response.json())
      .then(data => { resetSelect(subDistrictSelect, 'เลือกตำบล', false); data.forEach(item => { const option = document.createElement('option'); option.value = item.id; option.textContent = item.name_th; option.dataset.zipCode = item.zip_code || ''; subDistrictSelect.appendChild(option); }); refreshAddressDropdown(subDistrictSelect); })
