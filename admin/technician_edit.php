...
function get_technician_by_id(mysqli $conn, string $tech_id): ?array
{
    $stmt = $conn->prepare("SELECT * FROM technicians WHERE tech_id = ? LIMIT 1");
...
$technician = get_technician_by_id($conn, $tech_id);
    ...
    try {
        ...
        if ($check_result->num_rows > 0) {
            ...
            if ($duplicate['tech_name'] === $tech_name) {
                redirect_to(app_system_url('admin/technician_edit.php?id=' . urlencode($tech_id) . '&status=duplicate_name'));
            }

            if ($duplicate['tech_phone'] === $tech_phone) {
                redirect_to(app_system_url('admin/technician_edit.php?id=' . urlencode($tech_id) . '&status=duplicate_phone'));
            }

            if ($duplicate['tech_email'] === $tech_email) {
                redirect_to(app_system_url('admin/technician_edit.php?id=' . urlencode($tech_id) . '&status=duplicate_email'));
            }

            redirect_to(app_system_url('admin/technician_edit.php?id=' . urlencode($tech_id) . '&status=duplicate'));
        }
...
<div class="staff-form-page">
    <div class="staff-page-back-row"><a class="staff-back-link"
            href="<?= h(app_system_url('admin/technicians.php')) ?>"><?= admin_form_icon_svg('back') ?>
            กลับรายการช่าง</a></div>
    <form class="staff-create-card" method="POST" action="<?= h(app_system_url('admin/technician_edit.php')) ?>"
        autocomplete="off">
        ...
        <div class="staff-create-head staff-create-head-clean">
            ...
            <div class="staff-password-pill"><?= admin_form_icon_svg('lock') ?> เว้นรหัสผ่านว่างไว้ หากไม่ต้องการเปลี่ยน
            </div>
        </div>
        <div class="staff-form-grid">
            <div class="staff-field readonly-field"><label for="tech_id_show">รหัสช่าง *</label>
                <div class="staff-input-wrap"><?= admin_form_icon_svg('id') ?><input type="text" id="tech_id_show"
                        value="<?= h($technician['tech_id']) ?>" readonly></div>
            </div>
            <div class="staff-field"><label for="tech_status">สถานะช่าง *</label>
                <div class="staff-input-wrap select-wrap"><?= admin_form_icon_svg('role') ?><select id="tech_status"
                        name="tech_status" required><?php $current_status = (string) $technician['tech_status']; ?>
                        <option value="0" <?= $current_status === '0' ? 'selected' : '' ?>>0: พร้อมรับงาน</option>
                        <option value="1" <?= $current_status === '1' ? 'selected' : '' ?>>1: ไม่พร้อมรับงาน</option>
                    </select></div>
            </div>
            <div class="staff-field"><label for="tech_name">ชื่อผู้ใช้ *</label>
                <div class="staff-input-wrap"><?= admin_form_icon_svg('user') ?><input type="text" id="tech_name"
                        name="tech_name" maxlength="50" value="<?= h($technician['tech_name']) ?>" required></div>
            </div>
            <div class="staff-field"><label for="tech_fullname">ชื่อ-นามสกุลจริง *</label>
                <div class="staff-input-wrap"><?= admin_form_icon_svg('user') ?><input type="text" id="tech_fullname"
                        name="tech_fullname" maxlength="100" value="<?= h($technician['tech_fullname'] ?? '') ?>"
                        required></div>
            </div>
            <div class="staff-field"><label for="tech_phone">เบอร์โทรศัพท์ *</label>
                <div class="staff-input-wrap"><?= admin_form_icon_svg('phone') ?><input type="text" id="tech_phone"
                        name="tech_phone" maxlength="10" inputmode="numeric" value="<?= h($technician['tech_phone']) ?>"
                        required></div>
            </div>
            <div class="staff-field"><label for="tech_email">อีเมล Gmail *</label>
                <div class="staff-input-wrap"><?= admin_form_icon_svg('mail') ?><input type="email" id="tech_email"
                        name="tech_email" maxlength="50" value="<?= h($technician['tech_email']) ?>" required></div>
            </div>
            <div class="staff-field staff-field-full"><label for="tech_password">รหัสผ่าน</label>
                <div class="staff-input-wrap"><?= admin_form_icon_svg('lock') ?><input type="password"
                        id="tech_password" name="tech_password" maxlength="50"
                        placeholder="กรอกรหัสผ่านใหม่เมื่อต้องการเปลี่ยน"></div>
            </div>
        </div>
        <div class="staff-form-actions"><a class="staff-cancel-btn"
                href="<?= h(app_system_url('admin/technicians.php')) ?>">ยกเลิก</a><button class="staff-save-btn"
