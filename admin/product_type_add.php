...
function make_protype_id(mysqli $conn): string
{
    $prefix = 'PT-';

    for ($i = 1; $i <= 9999999; $i++) {
        $running_no = str_pad((string) $i, 7, '0', STR_PAD_LEFT);
        $protype_id = $prefix . $running_no;
    ...
    throw new Exception('ไม่สามารถสร้างรหัสประเภทสินค้าใหม่ได้');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (
        ...
        redirect_to(app_system_url('admin/product_type_add.php?status=error'));
    }

    if (!preg_match('/^PT-[0-9]{7}$/', $protype_id)) {
        redirect_to(app_system_url('admin/product_type_add.php?status=error'));
    }
    ...
    try {
        ...
        if ($check_result->num_rows > 0) {
            ...
            if ($duplicate['protype_id'] === $protype_id) {
                redirect_to(app_system_url('admin/product_type_add.php?status=duplicate_id'));
            }

            if ($duplicate['protype_name'] === $protype_name) {
                redirect_to(app_system_url('admin/product_type_add.php?status=duplicate_name'));
            }

            redirect_to(app_system_url('admin/product_type_add.php?status=duplicate'));
        }

        $stmt = $conn->prepare("
            INSERT INTO product_type
            (protype_id, protype_name, protype_detail)
            VALUES (?, ?, ?)
        ");
        ...
        redirect_to(app_system_url('admin/product_types.php?status=created'));
    } catch (Throwable $e) {
        redirect_to(app_system_url('admin/product_type_add.php?status=error'));
    }
}

$default_protype_id = make_protype_id($conn);
