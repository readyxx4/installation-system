...
if ($setup_id === '' || $tech_id === '') {
    redirect_to(app_system_url('technician/my_jobs.php?status=error'));
}
...
$stmt = $conn->prepare("
    ...
    LIMIT 1
");
$stmt->bind_param('ss', $tech_id, $setup_id);
$stmt->execute();
$setup = $stmt->get_result()->fetch_assoc();

if (!$setup) {
    redirect_to(app_system_url('technician/my_jobs.php?status=notfound'));
}
...
$slip_back_url = app_system_url('technician/job_detail.php?id=' . urlencode((string) ($setup['assign_id'] ?? '')));

layout_header('ใบติดตั้ง', 'my_jobs');
?>
...
<link rel="stylesheet"
    href="<?= h(app_asset_url('technician/assets/css/setup_slip.css')) ?>?v=<?= h(asset_version('technician/assets/css/setup_slip.css')) ?>">
